<!DOCTYPE html>
<html>
<head lang="zh-cn">
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="description" content="">
  <meta name="keywords" content="">
  <meta name="viewport" content="width=device-width,initial-scale=1">
  <title>Amaze UI React</title>
  <meta name="renderer" content="webkit">
  <!-- No Baidu Siteapp-->
  
  <!--
  <meta http-equiv="Cache-Control" content="no-siteapp"/>
  <link rel="alternate icon" type="image/png" href="i/favicon.png">
  <link rel="apple-touch-icon-precomposed" href="i/app-icon72x72@2x.png">
  <meta name="apple-mobile-web-app-title" content="AMUI React"/>
  <meta name="apple-mobile-web-app-capable" content="yes">
  <meta name="apple-mobile-web-app-status-bar-style" content="black">
  <link rel="stylesheet" href="http://cdn.amazeui.org/amazeui/2.5.0/css/amazeui.min.css"/>
  <script src="http://cdn.amazeui.org/react/0.14.3/react.min.js"></script>
  <script src="http://cdn.amazeui.org/react/0.14.3/react-dom.min.js"></script>
  <script src="http://cdn.amazeui.org/amazeui-react/1.2.3/amazeui.react.min.js"></script>
  -->
</head>
<body>
	<div>
	</div>
	<ul>
		<li><a>生成Java发布脚本</a></li>
		<div id="root"></div><div id="root2"></div>
		
		<div id="son"></div><div id="father"></div>
		
		
	</ul>
	<script type="text/babel" src="/static/main/main.js"></script>
	
</body>
<script src="/plug/jquery/jquery.min.js"></script>
<script src="/plug/react/react.min.js"></script>
<script src="/plug/react/react-dom.min.js"></script>
<script src="/plug/react/babel.min.js"></script>

<link rel="stylesheet" href="/static/main/main.css">
</html>